...
const COLOR_PALETTE = Object.freeze({
    ...
    color9: 0x4895ef,
    color10: 0x4cc9f0,
    color11: 0xFFA500, //yellow-orange
    color12: 0x505050,    // Darker Gray (Concrete-like)
    color13: 0xADD8E6     // Light Blue
});
...
const COLOR_PALETTE_GLSL = Object.freeze({
    ...
    color11: "vec4(1.0, 0.647, 0.0, 1.0)", //yellow-orange
    color12: "vec4(0.314, 0.314, 0.314, 1.0)", // Darker Gray (Concrete-like)
    color13: "vec4(0.678, 0.847, 0.902, 1.0)" // Light Blue
});

class DefaultMaterial extends THREE.MeshStandardMaterial {
    constructor() {
        super({
            color: COLOR_PALETTE.color11
        });
    }
}
        ...
        shader.fragmentShader = shader.fragmentShader.replace(
            ...
            if (isInRoad) {
                ...
                if (isInLine || isInDashedLine) {
                    diffuseColor = ${COLOR_PALETTE_GLSL.color11};
                }
            }
            `
        );
...
class BuildingMaterialA extends CustomMaterial {
    onBeforeCompile(shader) {
        ...
        shader.fragmentShader = shader.fragmentShader.replace(
            "#include <map_fragment>",
            `
            diffuseColor = ${COLOR_PALETTE_GLSL.color12};
            ...
            if (isInWindow) {
                diffuseColor = ${COLOR_PALETTE_GLSL.color13};
